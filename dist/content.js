(() => {
  ...
  var UNWRAP_TAGS = /* @__PURE__ */ new Set(["section"]);
  ...
  function prepareDocumentForReadability(clonedDocument) {
    clonedDocument.querySelectorAll("script, noscript, style, iframe, svg, canvas, form, dialog").forEach((element) => element.remove());
  ...
  function sanitizeArticleHtml(html, options) {
    ...
    const sanitizedHtml = Array.from(body.childNodes).map((node) => sanitizeNode(node, state, { inPre: false })).join("").trim();
    return {
  ...
  function sanitizeNode(node, state, context) {
    if (node.nodeType === Node.TEXT_NODE) {
      const text = context.inPre ? node.textContent || "" : (node.textContent || "").replace(/\s+/g, " ");
    ...
    if (!ALLOWED_TAGS.has(tagName)) {
      return Array.from(node.childNodes).map((child) => sanitizeNode(child, state, context)).join("");
    }
    ...
    if (tagName === "img") {
      ...
      const attributes2 = [`src="../${escapeAttribute(imageAsset.path)}"`];
      const altText = node.getAttribute("alt") || "";
      if (altText) {
        attributes2.push(`alt="${escapeAttribute(altText)}"`);
      }
      const width = numericAttribute(node.getAttribute("width"));
      const height = numericAttribute(node.getAttribute("height"));
      if (width) {
        attributes2.push(`width="${width}"`);
      }
      if (height) {
        attributes2.push(`height="${height}"`);
      }
      return `<img ${attributes2.join(" ")} />`;
    }
    const attributes = [];
    ...
    const content = Array.from(node.childNodes).map((child) => sanitizeNode(child, state, childContext)).join("");
  ...
  function readPathnameTitle(url) {
    try {
      ...
      const segments = parsed.pathname.split("/").map((segment) => segment.trim()).filter(Boolean).map((segment) => {
        try {
      ...
      const lastSegment = [...segments].reverse().find((segment) => /[A-Za-z\u4e00-\u9fff]/.test(segment));
      return (lastSegment || "").replace(/\.[a-z0-9]{1,5}$/i, "").replace(/[-_]+/g, " ").replace(/\s+/g, " ").trim();
    } catch {
      return "";
    }
  }
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    }).then((article) => sendResponse({ ok: true, article })).catch((error) => {
